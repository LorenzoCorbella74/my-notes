...
-  mettere dentro lo store ___solo lo stato condiviso tra + componenti___ (mandatorio quindi non mettere lo stato dei form o lo stato di elementi non serializzabili (come il router))
...
***DA RICORDARSI SEMPRE QUANDO SI COSTRUISCONO I COMPONENTI***: CHIEDERSI QUALE STATO SI VUOLE CONSERVARE IN MEMORIA ALL'USCITA DALLA PAGINA/COMPONENTE
      ...
      .pipe(
        ...
        mergeMap(() => this.productService.getProducts()
          .pipe(
            ...
            catchError(error => of(ProductActions.loadProductsFailure({ error }))) // fail action
          )
        )
      );
  });
...
```html
<button [ngClass]="{'active': product?.id=== (selectedProduct$ | async)?.id}"></button>
```
